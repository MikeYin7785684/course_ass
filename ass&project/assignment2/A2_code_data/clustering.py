...
def visualise_clusters(X, clusters, title=None, fout=None):
    ...
    if isinstance(X, np.ndarray):
        ...
        assert False
    t_sne = TSNE(metric=cosine_distance, init='pca', random_state=101)
...
def cluster_reviews_w2v(Xr, K, max_iter, rng=None):
    ...
    X_r=[get_sentences(xr) for xr in tqdm(Xr)]
    best_params=dict(window=50,vector_size=200)
    word_vectors=train_w2v(X_r,**best_params)
    X=get_features_w2v(X_r,word_vectors)
    new_centroids, clusters = kmeans(X, K, max_iter=max_iter, rng=rng)
...
if __name__ == '__main__':
    ...
    np.random.seed(100)
    rg=np.random.choice(50000,50000,replace=False)
    Xr = df.loc[df.index[rg]]["text"].tolist()
    K = 5
    max_iter = 20
    seed = 100
